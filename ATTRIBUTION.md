...
There was no pre-existing open-source parser for Agilent MassHunter `.d`
internals to draw on; the binary formats documented in
[docs/format/](docs/format/) were reverse-engineered from scratch against
the public PRIDE corpus (see [CORPUS.md](CORPUS.md)).

One piece of genuine prior art did make `MSProfile.bin` decodable:
`MSProfile.bin` uses standard, unmodified LZF compression (no Agilent-
specific framing) on its intensity payload.

### LZF

Marc Lehmann's LZF compression algorithm and reference implementation
(`liblzf`). The public LZF opcode format - not anything Agilent-specific -
is what made `MSProfile.bin`'s payload decodable.

Source: http://oldhome.schmorp.de/marc/liblzf.html
